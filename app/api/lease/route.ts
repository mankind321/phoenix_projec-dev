...
export async function POST(req: Request) {
  try {
    console.log("🟡 Create Lease API HIT");
    ...
    // 5️⃣ Create RLS Supabase client
    const supabase = createRlsClient(rlsHeaders);

    // 6️⃣ Verify property exists
    ...
    // 7️⃣ Create lease shell
    //
    // Only property_id is supplied.
    //
    // lease_id should be generated by the database.
    // Other tenant/lease information will be entered
    // through /dashboard/leases/[id].
    //
    ...
    if (leaseError) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: "Failed to create tenant lease.",
    ...
    // 8️⃣ Return the newly-created lease ID
    return NextResponse.json(
      {
        success: true,
        message: "Tenant lease created successfully.",
        lease: {
    ...
    console.error("🔥 Create Lease API Fatal Error:", err);
